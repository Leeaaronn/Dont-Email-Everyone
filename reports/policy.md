...
**The shared control group is resampled once per replicate, and that property is verified rather than asserted.** ROADMAP criterion 2 requires it, because the two email arms are measured against the *same* 10,653 control customers of the 32,001-row holdout: drawing that control group independently for each arm inside the same replicate would throw away the correlation between the two arms and make the interval on any difference between them wrong — wider or narrower, with nothing raised either way. The construction that avoids it is the one named above: **one** position-preserving three-level resample matrix over all 32,001 holdout rows, from which each arm's frame is recovered by *masking columns* by the original `segment`. Because the mask selects columns rather than re-drawing them, the control columns are the same columns in both arms' views by construction. `tests/test_evaluation.py::test_shared_control_is_drawn_once_per_replicate` asserts that equality on the committed artifact, derives both frame sizes from the masks rather than typing them, and then builds the forbidden one-matrix-per-arm alternative and shows it disagrees — so the test fails if the property is ever lost, rather than passing on arithmetic that would hold either way.
...
**Two `ate` artifacts exist and they are not interchangeable.** `data/processed/ate.parquet` is the six-row table — one row per arm and outcome, carrying the effect, its HC3 interval, the Holm-adjusted variant and both arm counts — and it is the artifact the pipeline actually *reads*: `pipeline.train()` and `pipeline.policy()` both open it by name, and a missing file raises there rather than surfacing later as a wrong number. `data/processed/ate.json` is the scalar sibling `analyze` writes beside it, for quoting without a Parquet read; its `effects` key is a row-for-row serialization of the same table, so the six effects are identical in both. This document and `reports/model.md` cite the Parquet throughout, because that is what the code on the path to these numbers opens; `reports/metric.md` cites the JSON for its endpoint cross-check, which is the file that comparison was written against. ROADMAP criterion 4 names the JSON in its artifact list. Either file discharges the criterion's requirement — both are committed, both are small and format-stable — but a reader tracing *this* phase's sign argument in section 6 should open the Parquet.
...
**The completeness of the first two claims is itself pinned**, because "every public function leaves the directory empty" narrows silently to "the functions that existed when the test was written" the moment someone adds a fifth. `tests/test_economics.py::test_economics_public_surface_is_pinned` asserts the exact public function list and its failure message says to extend the call list in the same commit; `tests/test_evaluation.py::test_evaluation_module_writes_nothing` carries the equivalent check inline, comparing the set of functions it called against the module's live public surface. A new public function therefore fails a test rather than quietly escaping the purity guarantee.

Run the three together with:

    .venv/Scripts/python.exe -m pytest tests/test_evaluation.py::test_evaluation_module_writes_nothing tests/test_economics.py::test_economics_module_writes_nothing tests/test_no_network.py::test_package_does_not_import_streamlit -q
