...
- [x] 05-06-PLAN.md — pipeline.policy() and the four committed artifacts: policy_curve.parquet, policy_bands.parquet, cost_sweep.parquet and manifest.json (wave 3)
...
**Success Criteria** (what must be TRUE):
  ...
  1. A threshold slider (top-k% by predicted uplift, shown as both a percentage and a customer count) updates the headline incremental-revenue-versus-a-random-send-of-the-same-size metric, with a policy/ranking selector alongside it over the published cells only, and the versus-emailing-everyone difference is also displayed. *(Amended 2026-09-10, same treatment the Phase 5 goal and its criterion 1 received. The original said the slider updates "the headline incremental-revenue-versus-emailing-everyone metric per treatment arm, with an arm/policy selector alongside it" — two clauses that locked decisions had already overtaken. First, D-08a replaced the comparator project-wide with a random send of the same size. Re-measured from the committed `policy_bands.parquet`: across all 909 band rows the versus-everyone interval excludes zero from above at 0 of 909 depths, and at the pre-registered anchor on the headline ranking it is -$0.236284, 95% [-$0.525647, +$0.102053] — so an app headlining it would display a negative figure as the result. That contrast is still computed, still in the artifact and still displayed: it appears with its 95% interval at the selected depth in the app's contrasts table, so the requirement's letter is met while D-08a's decision that it is not the headline is enforced by construction rather than by memory — a table cell is not an `st.metric`, and the app's `st.metric` count is capped at three. Second, a metric "per treatment arm" and the arm selector are forbidden by D-03 and D-04, and `policy_curve.parquet` carries no mens ranking at all: its three rankings are `uplift_womens_visit`, `uplift_womens_conversion` and one unproven cell that D-01 excludes. The policy selector is delivered; the arm selector cannot be.)*
...
- [x] 06-02-PLAN.md — D-04: relocate `OUTCOMES` and `SMD_THRESHOLD` into `config.py` with identity-preserving re-export shims, dropping statsmodels and scipy from `plots.py`'s import closure (wave 1)

**Wave 2** *(blocked on Wave 1 — same file)*

- [x] 06-03-PLAN.md — `selected=` marker on `policy_curve_plot` with `_POLICY_SELECTED_COLOUR`, its artifact-value introspection test, and D-06's full-pipeline regeneration gate (wave 2)
...
- [x] 06-05-PLAN.md — The D-07 headline block with three verdict states, the two policy curves, and the adjacency / verdict-versus-hatching tests (wave 4)

**Wave 5** *(blocked on Wave 4 — same file)*

- [x] 06-06-PLAN.md — The contrasts table carrying all three criterion-1 differences, the assumptions section with the live cost exhibit, the footer, and their tests (wave 5)
...
- [x] 06-08-PLAN.md — Deployment readiness gate, the Community Cloud deploy checkpoint, and the live link in the README (wave 7) *(Completed 2026-09-11. The app is live at https://dont-email-everyone-hillstrom.streamlit.app/ — the plan's proposed plain `dont-email-everyone` subdomain is taken by another app, so the plan's own documented fallback was used. The README carries the link and the sleep-and-wake note, guarded by `test_readme_carries_the_live_app_link` matching the URL by shape with a demonstrated negative control. **Two deploy-time observations were NOT captured and are recorded as UNVERIFIED rather than as passes:** the Python version selected in Advanced settings, and the build-log check that none of scikit-learn / statsmodels / duckdb / pandera was installed — threat T-06-31, whose failure mode is silent by construction, so an unperformed check is not a clean one. Both are handed to 06-09 and must be discharged before this phase closes. Criterion 5's README clause is met; its recorded-human-check-at-least-12-hours-after-last-traffic clause is 06-09's and is outstanding — last known traffic 2026-09-11T21:04Z.)*
