...
| 03-05-T3 | 03-05 | 4 | UPLIFT-02 / D-07 | — | N/A | unit | `pytest tests/test_evaluation.py -k bootstrap_indices -x` | ✅ | ✅ green |
| 03-05-T3 | 03-05 | 4 | UPLIFT-02 / D-06 | — | N/A | statistical | `pytest tests/test_evaluation.py -k bands -x` | ✅ | ✅ green |
| 03-05-T3 (added) | 03-05 | 4 | UPLIFT-02 / D-07 | T-03-19, T-03-20 | int32 dtype and the guarded int32 ceiling keep the matrix at 85 MB at R=500 and check the choice rather than assume it | unit | `pytest tests/test_evaluation.py::test_bootstrap_indices_shape_and_dtype -x` | ✅ | ✅ green |
| 03-05-T3 (added) | 03-05 | 4 | UPLIFT-02 / D-07 | — | Position-preserving arm membership — the invariant Phase 5 and Phase 6 index arbitrary per-row arrays under | unit | `pytest tests/test_evaluation.py::test_bootstrap_indices_preserves_arm_membership -x` | ✅ | ✅ green |
| 03-05-T3 (added) | 03-05 | 4 | UPLIFT-02 / D-02 | — | Same seed → identical matrix; different seed → different matrix, so the equality is not vacuous | unit | `pytest tests/test_evaluation.py::test_bootstrap_indices_is_seed_reproducible -x` | ✅ | ✅ green |
| 03-05-T3 (added) | 03-05 | 4 | UPLIFT-02 / D-07 | — | `replace=True` proven by a 60–67% distinct-index window; a lost flag would turn the bootstrap into a permutation | statistical | `pytest tests/test_evaluation.py::test_bootstrap_indices_actually_resamples_with_replacement -x` | ✅ | ✅ green |
| 03-05-T3 (added) | 03-05 | 4 | UPLIFT-02 / D-07 | T-03-20 | Degenerate arm raises `ValueError` naming the empty arm, via the shared `_guard_treatment` validator | unit | `pytest tests/test_evaluation.py::test_bootstrap_indices_rejects_a_degenerate_arm -x` | ✅ | ✅ green |
| 03-05-T3 (added) | 03-05 | 4 | UPLIFT-02 / D-06 | T-03-22 | The precomputed-matrix and self-generated code paths agree exactly at matching seeds | unit | `pytest tests/test_evaluation.py::test_bands_precomputed_indices_path_matches_self_generated -x` | ✅ | ✅ green |
| 03-05-T3 (added) | 03-05 | 4 | UPLIFT-02 / D-06 | T-03-23 | Both bands share one `(grid, lo, hi)` shape and one grid, so `qini_plot` has one band shape to draw | unit | `pytest tests/test_evaluation.py::test_bands_return_matching_grids tests/test_evaluation.py::test_bands_are_ordered -x` | ✅ | ✅ green |
| 03-05-T3 (added) | 03-05 | 4 | UPLIFT-02 / C2 | — | The D-06 payoff: an oracle curve escapes the random-score null band in the top 20% on synthetic data | statistical | `pytest tests/test_evaluation.py::test_oracle_curve_escapes_the_random_null_band_in_the_top_decile -x` | ✅ | ✅ green |
