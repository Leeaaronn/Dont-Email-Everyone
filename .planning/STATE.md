...
Last activity: 2026-09-12 - Completed quick task 260912-dvo: policy curve footprint cut 20% (height 7.5 -> 6.0 in), legend anchor re-based in inches, both shading states named
...
- ~~[Phase 6] The six-entry legend sits inside the plot area on both policy curves and overlaps the curve and the hatched covers-zero region.~~ **CLOSED 2026-09-11 by quick task 260911-l0p.** The premise recorded here was false, and the false premise is why it sat unfixed: D-06's gate is a REGENERATION DRIFT check (`pipeline all`, then `git status --short data/processed reports/figures` empty), NOT a byte-identity pin -- `tests/test_plots.py` documents that no committed-PNG checksum test exists, by repo-wide policy, and `06-UI-SPEC.md` asserted the opposite in five places. `plots.py` was never locked. The legend now sits BELOW the axes: a side legend costs ~half the on-screen plot width at ANY canvas width, because its apparent width scales with the type, while height is free since `st.pyplot` scales to column width. Point sizes carry a local 1.075 scale so apparent type holds at 1.000x, and the y-axis headroom that existed only to clear the in-axes legend is reclaimed.
- ~~[Phase 6] At k=20%, the app's first-paint depth, the purple pre-registered anchor was completely occluded by the green selection rule while the legend still advertised it with its own value and interval.~~ **CLOSED 2026-09-11 by quick task 260911-l0p**, found by opening renders during that task's checkpoint rather than by any test. The anchor is now drawn thick beneath a thinner selection rule, so it reads as a halo through the dash gaps; neither line moved and neither colour changed.
...
- **[Phase 6] OPEN. The policy figures have no maximum display width, so on a wide monitor they render ~2.9x the size their typography was calibrated for.** `streamlit_app.py:108` sets `layout="wide"` and `st.pyplot` renders `width="stretch"`, so the figure takes whatever the window gives it. `06-UI-SPEC.md:898` calibrates the type against **730 CSS px**; on a 2560 px viewport the figure displays at roughly **2090 CSS px**. Measured tick-label height is **12.67 px at 730** and **36.28 px at 2090**, against 16.00 px page body text -- identical for the 8.0in reference, the deployed 8.6x7.5 and the 8.6x6.0 that 260912-dvo ships, so this is NOT a figure-geometry defect and `_POLICY_FONT_SCALE` is doing exactly what it claims. At that width the figure is also ~1823 px tall, taller than a typical viewport, so the rotated y-axis label can only ever be read in fragments -- reported as "clipped mid-word", but it is oversize, not clipped. `layout="wide"` was adopted at the 06-07 checkpoint as the pre-approved remedy for the legend overlap (`06-UI-SPEC.md:405`), calibrated on one monitor; the 2026-09-11 amendment already downgraded it from fix to mitigation once `plots.py` was restyled. Remedy approved by the user 2026-09-12: cap the render at `st.pyplot(fig, width=730)`, which solves `1.075 x W/860 = 730/800` exactly and restores the apparent type the 05-08 checkpoint approved. Touches `streamlit_app.py`, so V18's rationale needs a dated amendment.
- ~~[Phase 6] `tests/test_app.py::test_headline_tracks_the_committed_curve` fails intermittently under full-suite load.~~ **CLOSED 2026-09-12 by debug session `apptest-timeout-headline-test`. Root cause: AppTest measures its per-run timeout on a WALL CLOCK.** `streamlit/testing/v1/local_script_runner.py:195-196` does `t0 = time.time()` then polls `while time.time() - t0 < timeout`. This machine steps its system clock forward after sleep/resume -- Windows Kernel-General id 1, **+66.5 s at 11:51:39, +245.5 s at 12:21:22, +2272.3 s at 13:56:13 on 2026-09-12 alone**, and +32,170 s earlier that morning. A step larger than the 60 s budget landing mid-poll expires the deadline against a healthy run. **The test never needs and never spends 60 s; no 60 seconds ever passes.** Evidence: 627 instrumented script runs show max **2.113 s**, p99 1.984 s, zero above 2.2 s -- no tail to explain, refuting both the stall and the gradual-slowness hypotheses; `tests/test_app.py` runs FIRST of twenty modules alphabetically, so no earlier test can create the condition, which eliminates memory pressure, figure-registry growth and cache state outright; and injecting one +66.5 s step reproduces the exact failure, taking the test from 18.0 s green to **10.4 s red** -- FASTER, because the time was never spent. That inversion was already in the record and misread as noise: the failing full-suite run took **575.2 s** while a passing one took **763.8 s**. **The earlier "1 in 3 at HEAD, 0 in 3 at a195222" rate was meaningless** -- it depends on whether the laptop slept during the run, not on the code, so this was never a regression from 260912-dvo. Fix: `tests/conftest.py` rebinds `require_widgets_deltas` to the identical loop on `time.monotonic()`. The budget stays 60 s and a genuinely hung run still fails at 60 s -- the INSTRUMENT changed, not the ALLOWANCE. Guarded by `tests/test_app.py::test_apptest_timeout_is_measured_on_an_elapsed_clock` with a shipped-loop negative control. **Not claimed:** no natural failure was captured in the wild; the mechanism is proven by injection plus the event log. Residual risk if `time.monotonic()` counts suspend time on this platform -- much narrower, and the new message reports measured elapsed seconds so a recurrence self-identifies. This is an upstream Streamlit defect affecting every AppTest user.
...
|----------|------|--------|-------------|
| Figure copy | The policy-curve legend's second line reads "Unhatched: the gain is detectable at **that** depth" where its first reads "...at **this** depth". One word, for parallelism. Needs a `pipeline all` run to reland the two PNGs, so it must ride along with a commit that regenerates them rather than spend a ~45-minute run alone. **Do NOT bundle it into 260911-wco** (the Python 3.14 upgrade): that task's whole purpose is a per-cell artifact diff against a git-show baseline, and changing legend text would make the policy curves differ for a reason unrelated to the interpreter, corrupting the gate. Land it immediately AFTER wco's gate passes, or as its own task. | Open | 2026-09-12 |
