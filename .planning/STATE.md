...
- **[Phase 6] OPEN, low severity. `tests/test_app.py::test_headline_tracks_the_committed_curve` is an intermittent failure under full-suite load.** Observed **1 failure in 3** full-suite runs at 260912-dvo's HEAD; **0 in 3** at the pre-change commit a195222. The failure is `RuntimeError: AppTest script run timed out after 60(s)` from `streamlit/testing/v1/local_script_runner.py:203` -- a wall-clock timeout on one of the test's ten `AppTest` reruns, not a logic error; the test passes alone in 9.4 s against the same 60 s budget. Duration does not predict it: the baseline's slowest run (763.8 s) passed while the post-change failing run (575.2 s) did not. Evidence is therefore inconclusive on whether 260912-dvo's ten added render-heavy parameterisations raised the rate; there is no evidence of a systematic regression. Re-check before Phase 7 closes, and treat a green single run as weak evidence.
