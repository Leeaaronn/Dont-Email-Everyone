...
- ~~[Phase 6] The policy figures have no maximum display width, so on a wide monitor they render ~2.9x the size their typography was calibrated for.~~ **CLOSED 2026-09-12 by quick task 260912-jil, human-verified on a wide monitor and at laptop width.** `layout="wide"` plus `st.pyplot`'s `width="stretch"` default let the browser window supply an UNBOUNDED scale factor: on a 2560 px viewport the figure rendered at roughly 2090 CSS px against type calibrated for 730, putting tick labels at 36.28 px beside 16.00 px page text, and making the figure ~1823 px tall so the rotated y-axis label could only be read in fragments -- reported as "clipped mid-word", but oversize rather than clipped. The cap is `730`, derived not chosen: solving `_POLICY_FONT_SCALE x (W / 860) = 730/800` gives W = 730.0 exactly. **The derivation collapses** -- `_POLICY_FONT_SCALE` is defined as `_POLICY_FIGSIZE_IN / 8.0`, so the canvas width cancels and the cap is right for any canvas width while that coupling holds; the guard test therefore pins the COUPLING, with a negative control, not the number. Streamlit sets the pixel width alongside `max-width: 100%` (verified in the shipped frontend bundle), so the integer width is an upper bound and narrow viewports cannot overflow. Shipped apparent type: **13.35 / 13.41 px** (policy curves) and **12.85 px** (cost exhibit) against 16.00 px body text. Those two families differ by 3.9% as shipped because `bbox_inches="tight"` crops 5.06% off the policy canvas and 1.38% off the cost canvas; equalising would mean a cap of 703, and the approved 730 was kept rather than overridden for half a pixel.
