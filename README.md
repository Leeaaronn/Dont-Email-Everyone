...
Reproduce on **Python 3.11**. The deployed app runs **Python 3.14.7**, which Streamlit Community Cloud selects for itself — neither figure corrects the other, and the deployment runs no part of the pipeline.

Why each is what it is: [`docs/decisions.md`](docs/decisions.md).
...
Engineering decisions — dependency pinning, the interpreter detail, and six reconciliations between plan and implementation — are recorded in [`docs/decisions.md`](docs/decisions.md).

## How this was built

The work ran in gated phases, and the gate was always the same: decisions were written down before the results they would govern existed. The targeting depth this project publishes at, and the bar a model had to clear to be published at all, were both fixed in advance — the bar was then applied once, to six candidate models, and four of them failed it and are reported as failures.

Every number in this README, in the app and in the four write-ups is derived from a committed artifact at test time rather than transcribed. Nothing here is a figure someone typed and nobody checked since.
